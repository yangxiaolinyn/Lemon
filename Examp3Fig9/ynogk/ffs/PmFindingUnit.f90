...
!*******************************************************************************        
      ...
      contains
!******************************************************************************
      function pemfind(f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,rin,rout,muup,mudown,&
                       phy1,phy2,caserange,Fp,paras,orir,oricosth,bisection, geop, geor, geomu)   
!******************************************************************************
...
!*     REVISIONS: ****************************************** 
        ...
        double precision, dimension(0:m,0:m,0:geoN), intent(in) :: geor, geomu, geop
...
!write(*,*)'p1,p2=',p1,p2 
            If(caserange.eq.4)then        
                pemfind=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,&
                        robs,scal,tr1,tr2,p1,p2,NN,Fp,paras,orir,oricosth,bisection, geop, geor, geomu)        
            else        
                pemfind=Sectionp(f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,&
                        tr1,tr2,p1,p2,muup,mudown,phy1,phy2,caserange,NN,Fp,&
                             paras,orir,oricosth,bisection, geop, geor, geomu)
            endif 
        case(3)
            tr1=0
            tr2=0        
            ...
            If(caserange.eq.4)then        
                pemfind=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,&
                                robs,scal,tr1,tr2,p1,p2,NN,Fp,paras,&
                                 orir,oricosth,bisection, geop, geor, geomu)        
            else        
                pemfind=Sectionp(f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,&
                        tr1,tr2,p1,p2,muup,mudown,phy1,phy2,caserange,NN,Fp,&
                           paras,orir,oricosth,bisection, geop, geor, geomu)
            ...
            If(pemfind.eq.-one)then
                ...
                If(caserange.eq.4)then        
                    pemfind=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,&
                                robs,scal,tr1,tr2,p1,p2,NN,Fp,paras,orir,&
                                 oricosth,bisection, geop, geor, geomu)        
                else        
                    pemfind=Sectionp(f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,tr1,tr2,p1,&
                               p2,muup,mudown,phy1,phy2,caserange,NN,Fp,&
                               paras,orir,oricosth,bisection, geop, geor, geomu)
...
!*****************************************************************************************************
      Function Sectionp(f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,&
                t1,t2,p1,p2,muup,mudown,phy1,phy2,caserange,NN,Fp,&
                    paras,orir,oricosth,bisection, geop, geor, geomu)  
!*****************************************************************************************************
...
!possibility to hit the surface of the object.        
                            Sectionp=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,&
                                        robs,scal,t1,t2,p1,p2,NN,Fp,paras,orir,&
                                        oricosth,bisection, geop, geor, geomu) !(1,1)
...
!the (phy1,phy2) and (phya1,phya2) has no public point,so we don't consider it.
                        ...
                        Sectionp=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,&
                                robs,scal,t1,t2,p1,p2,NN,Fp,paras,orir,&
                                  oricosth,bisection, geop, geor, geomu) !(1,0)
                    endif
                else 
...
!surface of the object at the internal (p_rout,p_rout2),which also means the geodesic will never hit the object.
                ...
                If(caserange.eq.1 .or.caserange.eq.3)then
                    If((phya1-phy1)*(phya1-phy2).lt.zero.or.(phya2-phy1)*(phya2-phy2).lt.zero.or.&
                    (phy1-phya1)*(phy1-phya2).lt.zero.or.(phy2-phya1)*(phy2-phya2).lt.zero)then
                        Sectionp=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,t1,t2,p1,p2,NN,Fp,&
                                   paras,orir,oricosth,bisection, geop, geor, geomu) !(0,1)
                    else 
!the (phy1,phy2) and (phya1,phya2) has no public points,so we don't consider it.
                    ...
                    Sectionp=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,&
                                robs,scal,t1,t2,p1,p2,NN,Fp,paras,orir,&
                                oricosth,bisection, geop, geor, geomu)        ! (0,0)        
                endif        
            endif
        return
...
!********************************************************************************* 
      Function rootfind(f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,&
                           t1,t2,p1,p2,NN,Fp,paras,orir,oricosth,bisection, geop, geor, geomu)
!********************************************************************************* 
...
!*     REVISIONS: ****************************************** 
      ...
      double precision, dimension(0:m,0:m,0:geoN), intent(in) :: geor, geomu, geop
        ...
        NNf=floor(NN)
        p=p1
        f_p=Fp(p,f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,t1,t2,&
                               paras,orir,oricosth, geop, geor, geomu)
        !write(*,*)'NNf=',f_p
        ...
        If(f_p.lt.zero)then
                ...
                Do while(.true.)			
                        ...
                        k=k+1
                        p=p1+deltap*k
                        f_p=Fp(p,f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,&
                                       t1,t2,paras,orir,oricosth, geop, geor, geomu)
 write(unit=6,fmt=*)'f_p=11111',f_p,k,p
                Enddo                                        
        else
                k=0        
                Do while(.true.)
...
! Using bisection or Newton Raphson method to find roots on interval (sp1, sp2).   
                    IF(bisection)THEN
                        rootfind=Bisectionp(f1234,lambda,q,sinobs,muobs,&
                                a_spin,robs,scal,t1,t2,sp1,sp2,Fp,paras,orir,oricosth, geop, geor, geomu)
                    else
...
!*****************************************************************************************************
      Function Bisectionp(f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,t1,t2,&
                                  p1,p2,Fp,paras,orir,oricosth, geop, geor, geomu)
!*****************************************************************************************************
...
!*     REVISIONS: ****************************************** 
      ...
      double precision, dimension(0:m,0:m,0:geoN), intent(in) :: geor, geomu, geop
        ...
        pc=(p1+p2)/two
        f1=Fp(p1,f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,t1,t2,&
                               paras,orir,oricosth, geop, geor, geomu)
        f2=Fp(p2,f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,t1,t2,&
                               paras,orir,oricosth, geop, geor, geomu)
        fc=Fp(pc,f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,t1,t2,&
                               paras,orir,oricosth, geop, geor, geomu)        
...
!*****************************************************************************************************
      Function NewRapson(f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,&
                           t1,t2,p1,p2,Fp,paras,orir,oricosth, geop, geor, geomu)
!*****************************************************************************************************
...
!*****************************************************************
      ...
      double precision, dimension(0:m,0:m,0:geoN), intent(in) :: geor, geomu, geop
        ...
        Do k=1,kmax
            ...
            pj=temp+h
            h=pj-temp
            f_p=Fp(temp,f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,t1,t2,&
                                    paras,orir,oricosth, geop, geor, geomu)
            f_pj=Fp(pj,f1234,lambda,q,sinobs,muobs,a_spin,robs,scal,t1,t2,&
                                    paras,orir,oricosth, geop, geor, geomu)
            dfp=(f_pj-f_p)/h        
            dp=f_p/dfp
            ptn=ptn-dp
