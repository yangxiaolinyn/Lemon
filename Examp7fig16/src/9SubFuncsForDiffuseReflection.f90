...
!**************************************************************************************
    SUBROUTINE Mimick_Photon_Diffuse_Transfer( Total_Phot_Num, tau, epsilon_crit, mu0, phi0, &
           I0, Q0, U0, V0, phi_esti, Num_phi_esti, MCResultsFNphi, ChandraFNphi )
!************************************************************************************** 
    implicit none
    real(mcp), intent(inout) :: tau, epsilon_crit, mu0, phi0, I0, Q0, &
                         U0, V0, phi_esti(1: Num_phi_esti)
    integer, intent(inout) :: Num_phi_esti
    ...
    real(mcp) :: IQUV_Recv(1: 4, 0: Num_mu, 1:2, 1: Num_phi_esti)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    integer :: i,j 
!--------------MPI---------------------------------------------------------------
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    CALL phot%Set_initial_parameter_values( tau, mu0, phi0, I0, &
                               Q0, U0, V0, phi_esti, Num_phi_esti ) 
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
        ...
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
            ...
            CALL phot%Determine_P_Of_Scatt_Site_And_Quantities_At_p( ) 
            if( phot%Psi_I <= epsilon_crit )exit
            !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~     
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
      If ( myid /= np-1 ) then  
          send_num = (Num_mu + 1) * Num_phi_esti * 4 * 2
          send_tag = 1  
          call MPI_SEND( phot%IQUV_Emerg, send_num, MPI_DOUBLE_PRECISION, np-1, &
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
          if (np-2 >= 0) then
              do RECV_SOURCE = 0, np-2  
                  ...
                  recv_num = (Num_mu + 1) * Num_phi_esti * 4 * 2
                  ...
                  phot%IQUV_Emerg = phot%IQUV_Emerg + IQUV_Recv   

              enddo
          endif   
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
          open(unit=9, file = MCResultsFNphi, status="replace")  
          do i = 1, phot%N_phi_esti
              do j = 0, Num_mu
                  write(unit = 9, fmt = 100)phot%IQUV_Emerg(1: 4, j, 2, i)
              enddo
              do j = Num_mu, 0, -1
                  write(unit = 9, fmt = 100)phot%IQUV_Emerg(1: 4, j, 1, i)
              enddo
          enddo
      ...
      deallocate( phot%IQUV_Emerg, stat = ierr )
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
